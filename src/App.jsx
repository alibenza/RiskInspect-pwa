...
import { 
  FileDown, 
  Menu, // Icône pour ouvrir le menu
  X,    // Icône pour fermer le menu
  ClipboardCheck, 
  BarChart3, 
  BrainCircuit,
  LogOut,
...
function App() {
  ...
  const [isSidebarOpen, setSidebarOpen] = useState(false); // Masqué par défaut
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 text-slate-900 font-sans relative overflow-x-hidden">
      ...
      {/* OVERLAY : Flou d'arrière-plan quand le menu est ouvert */}
      {isSidebarOpen && (
        <div 
      ...
      {/* SIDEBAR LATÉRALE (STYLE TIROIR COULISSANT) */}
      <aside 
      ...
      >
          ...
          {menuItems.map((item) => (
            <button
              ...
              onClick={() => {
                ...
                setSidebarOpen(false); // Ferme automatiquement après clic
              }}
        ...
        <div className="p-4 border-t border-slate-800 space-y-2">
          <button 
            ...
            className="w-full flex items-center gap-4 p-4 text-emerald-400 hover:bg-slate-800 rounded-2xl transition-all"
          >
            <FileDown size={20} />
