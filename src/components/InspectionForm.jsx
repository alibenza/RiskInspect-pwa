...
const InspectionForm = () => {
  ...
  return (
    <div className="space-y-4 pb-40 animate-in fade-in duration-500">
      ...
      {/* --- BARRE DE NAVIGATION DES SITES --- */}
      <div className="px-2 mb-4">
        <div className="bg-white rounded-3xl border border-slate-100 p-2 shadow-sm flex items-center gap-2 overflow-x-auto no-scrollbar">
          {Object.entries(allSites).map(([id, site]) => (
            <div key={id} className="relative group flex-shrink-0">
              <button
                onClick={() => switchSite(id)}
                className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-tight transition-all ${
                  activeSiteId === id 
              ...
              {Object.keys(allSites).length > 1 && (
                <button 
                  onClick={(e) => { e.stopPropagation(); if(confirm("Supprimer ce site ?")) deleteSite(id); }}
                  className="absolute -top-1 -right-1 bg-white text-red-500 rounded-full p-0.5 border border-slate-100 shadow-sm opacity-0 group-hover:opacity-100 transition-opacity"
                >
                  <X size={10} />
                </button>
              )}
          ...
          <button
            onClick={() => {
              const name = prompt("Nom de la nouvelle unité / site :");
              if (name) switchSite(`site_${Date.now()}`, name);
            }}
            className="flex-shrink-0 p-2.5 bg-emerald-50 text-emerald-600 rounded-2xl hover:bg-emerald-100 transition-colors"
          >
            <PlusCircle size={20} />
          </button>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 px-2 mb-2">
        <button
          type="button"
          onClick={() => exportAudit()}
          className="flex items-center justify-center gap-2 px-6 py-3 bg-slate-800 text-white rounded-xl font-bold text-xs uppercase hover:bg-slate-700 transition-all shadow-lg"
        >
          <FileJson size={16} />
          Exporter Global (.json)
        </button>
      ...
      {questionsConfig.map((section, sIdx) => {
        ...
        return (
          <div key={sIdx} className="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden transition-all text-left">
            <button 
              onClick={() => toggleSection(sIdx)}
              className={`w-full flex items-center justify-between p-6 transition-all ${isOpen ? 'bg-slate-50' : 'bg-white'}`}
            >
              <div className="flex items-center gap-4 text-left">
                <div className={`p-2 rounded-xl flex-shrink-0 ${complete ? 'bg-green-100 text-green-600' : 'bg-indigo-50 text-indigo-600'}`}>
            ...
            {isOpen && (
              <div className="p-6 pt-0 space-y-8 animate-in slide-in-from-top-2 duration-300">
                ...
                {section.questions.map((q) => (
                  <div key={q.id} className="space-y-4 border-b border-slate-50 pb-8 last:border-0 text-left">
                    <label className="block text-sm font-bold text-slate-800 leading-tight text-left">
                      {q.label}
                    </label>
