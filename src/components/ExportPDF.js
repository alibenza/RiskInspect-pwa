...
export const exportToPdf = async (responses, questionsConfig, aiResults, auditorInfo) => {
  try {
    ...
    infoRows.forEach(([lbl, val], i) => {
      ...
      // Correction Marge Droite Bloc Infos
    ...
    if (smpData?.valeurs) {
      ...
      doc.autoTable({
        startY: y,
        margin: { left: ML, right: ML }, // Correction Marges Tableaux
      ...
      if (smpData.scenario) {
        ...
        // Correction Marge Scénario
        const sc = doc.splitTextToSize(smpData.scenario, CW - 8);
    ...
    if (aiResults?.analyses_par_garantie?.length > 0) {
      ...
      if (aiResults.recommandations?.length > 0) {
        ...
        aiResults.recommandations.forEach((r, i) => {
          ...
          normal(doc, 8); rgb(doc, C.BODY);
          const ls = doc.splitTextToSize(r, CW - 12);
    ...
    for (const [siteId, site] of Object.entries(allSites)) {
      ...
      for (const sec of (questionsConfig || [])) {
        ...
        for (const q of qList) {
          ...
          // Correction Marge Libellé Question vs Score
          const labelMaxW = hasScore ? CW - 42 : CW - 6;
          ...
          if (r.value || r.comment?.trim()) {
            ...
            // Correction Marge Observation
            const ls = doc.splitTextToSize(text, CW - 12);
            normal(doc, 8); rgb(doc, C.BODY);
            doc.text(ls, ML + 4, y);
            y += ls.length * 5 + 2;
          }
    ...
    if (hasSmpContent) {
      ...
      if (smpData.scenario) {
        ...
        // Correction Marge Scénario (Fin Rapport)
      ...
      if (smpData.hypotheses?.length > 0) {
        ...
        smpData.hypotheses.forEach((h, i) => {
          ...
          // Correction Marge Hypothèses
          const hLines = doc.splitTextToSize(h, CW - 15);
    ...
    const drawSigBox = (x, label, name, role) => {
      ...
      bold(doc, 8); rgb(doc, C.NAVY); doc.text(name, x + 4, sigY + 17, { maxWidth: sigW - 8 });
      normal(doc, 7); rgb(doc, C.SUBTLE); doc.text(role, x + 4, sigY + 23, { maxWidth: sigW - 8 });
